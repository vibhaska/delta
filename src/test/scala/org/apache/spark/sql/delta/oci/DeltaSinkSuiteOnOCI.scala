...
import org.apache.spark.sql.oci.{FileOnOCI, StreamTestOnOCI}
import org.apache.spark.sql.streaming._
import org.apache.spark.sql.types._

class DeltaSinkSuiteOnOCI extends StreamTestOnOCI {

  override val streamingTimeout = 60.seconds
  import testImplicits._

  private def withTempDirOnOCIs(f: (FileOnOCI, FileOnOCI) => Unit): Unit = {
  ...
  test("append mode") {
    failAfter(streamingTimeout) {
      withTempDirOnOCIs { (outputDir, checkpointDir) =>
  ...
  test("path not specified") {
    failAfter(streamingTimeout) {
      withTempDirOnOCI { checkpointDir =>
        val inputData = MemoryStream[Int]
        val df = inputData.toDF()
  ...
  test("do not trust user nullability, so that parquet files aren't corrupted") {
    ...
    withTempDirOnOCI { base =>
      val sourceDir = new FileOnOCI(base, "source")
      val tableDir = new FileOnOCI(base, "output")
      val chkDir = new FileOnOCI(base, "checkpoint")

      writeOnOCI(new FileOnOCI(sourceDir, "a.json"), jsonRec)
      ...
      val q = spark.readStream
        ...
        .option("checkpointLocation", chkDir.getCanonicalPath)
        .start(tableDir.getCanonicalPath)

      q.awaitTermination()

      checkAnswer(
        spark.read.format("delta").load(tableDir.getCanonicalPath).drop("file"),
