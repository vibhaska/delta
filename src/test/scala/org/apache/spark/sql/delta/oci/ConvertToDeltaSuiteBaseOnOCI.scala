...
import org.apache.spark.sql.delta.{DeltaLog, DeltaTable, S3LikeLocalFileSystem}
 ...
 */
trait ConvertToDeltaTestUtilsOnOCI extends QueryTestOnOCI { self: SQLTestUtils =>
...
trait ConvertToDeltaSuiteBaseOnOCI extends ConvertToDeltaTestUtilsOnOCI
  with SharedSparkSession
  with SQLTestUtils
  with ConvertToDeltaHiveTableTestsOnOCI
  ...
  // For OCI Object Store removed '(' and ')'
  " ,;{}\n\t=".foreach { char =>
  ...
  // For OCI Object Store removed '+', '<', '>', '_' and '|'
  testSpecialCharactersInDirectoryNames("?./", expectFailure = false)
 ...
 */
trait ConvertToDeltaHiveTableTestsOnOCI extends ConvertToDeltaTestUtilsOnOCI with SQLTestUtils {
