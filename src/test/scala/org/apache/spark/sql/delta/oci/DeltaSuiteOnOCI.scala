...
class DeltaSuiteOnOCI extends QueryTestOnOCI
  ...
  test("move delta table") {
    val tempDir = createTempDirOnOCI()
    val targetDir = new FileOnOCI(createTempDirOnOCI(), "target")
    try {
      ...
      assert(renameToOnOCI(tempDir, targetDir))
      ...
      deleteRecursivelyOnOCI(tempDir)
      deleteRecursivelyOnOCI(targetDir)
    }
  }
  ...
  test("write via data source API when the directory doesn't exist") {
    withTempDirOnOCI { tempDir =>
      ...
      val q = input.toDF
        ...
        .option(
          "checkpointLocation",
          createTempDirOnOCI(namePrefix = "tahoe-test").getCanonicalPath)
  ...
  test("special chars in base path") {
    withTempDirOnOCI { dir =>
      val basePath = new FileOnOCI(new FileOnOCI(dir, "some space"), "and#spec*al+ch@rs")
  ...
  test("SC-24982 - initial snapshot does not trigger jobs") {
    ...
    try {
      withTempDirOnOCI { tempDir =>
        val files = DeltaLog.forTable(spark, tempDir.path).snapshot.state.collect()
