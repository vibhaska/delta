...
class DeltaTimeTravelSuiteOnOCI extends QueryTestOnOCI
  ...
  private def modifyCommitTimestamp(deltaLog: DeltaLog, version: Long, ts: Long): Unit = {
    val file = new FileOnOCI(FileNames.deltaFile(deltaLog.logPath, version).toUri)
    setLastModifiedOnOCI(file, ts)
    val crc = new FileOnOCI(FileNames.checksumFile(deltaLog.logPath, version).toUri)
    if (existsOnOCI(crc)) {
      setLastModifiedOnOCI(crc, ts)
    }
  }

  private def modifyCheckpointTimestamp(deltaLog: DeltaLog, version: Long, ts: Long): Unit = {
    val file = new FileOnOCI(FileNames.checkpointFileSingular(deltaLog.logPath, version).toUri)
    setLastModifiedOnOCI(file, ts)
  }
  ...
  private def generateCommits(location: String, commits: Long*): Unit = {
    ...
    commits.foreach { ts =>
      ...
      val file = new FileOnOCI(FileNames.deltaFile(deltaLog.logPath, startVersion).toUri)
      setLastModifiedOnOCI(file, ts)
      startVersion += 1
    }
  }
  ...
  private def historyTest(testName: String)(f: DeltaLog => Unit): Unit = {
    testQuietly(testName) {
      withTempDirOnOCI { dir => f(DeltaLog.forTable(spark, dir.path)) }
    }
  }

  private def ignoreTest(testName: String)(f: DeltaLog => Unit): Unit = {
    ignore(testName) {
      withTempDirOnOCI { dir => f(DeltaLog.forTable(spark, dir.path)) }
    }
  }

  // @see org.apache.hadoop.fs.FileSystem.setTimes
  // Modify TimeStamps doesn't work in OCI's Object Store
  ignoreTest("getCommits should monotonize timestamps") { deltaLog =>
  ...
  // @see org.apache.hadoop.fs.FileSystem.setTimes
  // Modify TimeStamps doesn't work in OCI's Object Store
  ignoreTest("describe history timestamps are adjusted according to file timestamp") { deltaLog =>
  ...
  // @see org.apache.hadoop.fs.FileSystem.setTimes
  // Modify TimeStamps doesn't work in OCI's Object Store
  ignoreTest("should filter only delta files when computing earliest version") { deltaLog =>
    val start = 1540415658000L
  ...
  // @see org.apache.hadoop.fs.FileSystem.setTimes
  // Modify TimeStamps doesn't work in OCI's Object Store
  ignoreTest("resolving commits should return commit before timestamp") { deltaLog =>
    val start = 1540415658000L
  ...
  // @see org.apache.hadoop.fs.FileSystem.setTimes
  // Modify TimeStamps doesn't work in OCI's Object Store
  ignore("log cleanup should handle adjusted commit timestamps") {
    withTempDirOnOCI { dir =>
      ...
      val deltaLog = DeltaLog.forTable(spark, new Path(dir.toString), clock)
  ...
  // @see org.apache.hadoop.fs.FileSystem.setTimes
  // Modify TimeStamps doesn't work in OCI's Object Store
  ignore("as of timestamp in between commits should use commit before timestamp") {
    withTempDirOnOCI { dir =>
  ...
  // @see org.apache.hadoop.fs.FileSystem.setTimes
  // Modify TimeStamps doesn't work in OCI's Object Store
  ignore("as of timestamp on exact timestamp") {
    withTempDirOnOCI { dir =>
      val tblLoc = dir.getCanonicalPath
      val start = 1540415658000L
  ...
  // @see org.apache.hadoop.fs.FileSystem.setTimes
  // Modify TimeStamps doesn't work in OCI's Object Store
  ignore("as of exact timestamp after last commit should fail") {
    withTempDirOnOCI { dir =>
      val tblLoc = dir.getCanonicalPath
      val start = 1540415658000L
  ...
  test("as of with versions") {
    withTempDirOnOCI { dir =>
      ...
      deleteOnOCI(new FileOnOCI(FileNames.deltaFile(deltaLog.logPath, 0).toUri))
      val e2 = intercept[AnalysisException] {
  ...
  // @see org.apache.hadoop.fs.FileSystem.setTimes
  // Modify TimeStamps doesn't work in OCI's Object Store
  ignore("time travelling with adjusted timestamps") {
    withTempDirOnOCI { dir =>
      val tblLoc = dir.getCanonicalPath
      val start = 1540415658000L
  ...
  test("don't time travel a valid delta path with @ syntax") {
    withTempDirOnOCI { dir =>
      val path = new FileOnOCI(dir, "base@v0").getCanonicalPath
