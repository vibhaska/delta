...
trait DeltaVacuumSuiteBaseOnOCI extends QueryTestOnOCI
  ...
  test("partition column name starting with underscore") {
    ...
    withEnvironmentOnOCI { (tempDir, clock) =>
      ...
      gcTestOnOCI(deltaLog, clock)(
        CreateFile("file1.txt", commitToActionLog = true,
          partitionValues = Map("_underscore_col_" -> "10")),
        ...
        // Time changed to match OCI File creation/modification time i.e. System time
        AdvanceClock(System.currentTimeMillis() + defaultTombstoneInterval + 100000),
  ...
  testQuietly("correctness test") {
    withEnvironmentOnOCI { (tempDir, clock) =>
      ...
      val reservoirDir = new FileOnOCI(tempDir, "reservoir")
      assert(mkdirOnOCI(reservoirDir))
      val externalDir = new FileOnOCI(tempDir, "external")
      assert(mkdirOnOCI(reservoirDir))
      val deltaLog = DeltaLog.forTable(spark, reservoirDir, clock)

      val externalFile = new FileOnOCI(externalDir, "file4.txt").getAbsolutePath

      gcTestOnOCI(deltaLog, clock)(
        // Time changed to match OCI File creation/modification time i.e. System time
        AdvanceClock(System.currentTimeMillis()),
        ...
        CheckFiles(Seq("file3.txt")),
        AdvanceClock(120000),
        GC(dryRun = true, Seq(new FileOnOCI(reservoirDir, "file3.txt").toString)),
  ...
  test("deleting directories") {
    withEnvironmentOnOCI { (tempDir, clock) =>
      ...
      gcTestOnOCI(deltaLog, clock)(
        ...
        GC(dryRun = true, Seq(new FileOnOCI(tempDir, "ghi"))),
        GC(dryRun = false, Seq(tempDir)),
  ...
  // Doesn't work with OCI
  ignore("deleting files with special characters in path") {
    withEnvironmentOnOCI { (tempDir, clock) =>
      val deltaLog = DeltaLog.forTable(spark, tempDir.getAbsolutePath, clock)
      gcTestOnOCI(deltaLog, clock)(
        ...
        AdvanceClock(defaultTombstoneInterval + 1000),
        GC(dryRun = true, Seq(new FileOnOCI(tempDir, "abc def/#1/file2.txt"))),
        GC(dryRun = false, Seq(tempDir)),
  ...
  protected def withEnvironmentOnOCI(f: (FileOnOCI, ManualClock) => Unit): Unit = {
    withTempDirOnOCI { file =>
  ...
  implicit def fileToPathString(f: FileOnOCI): String = new Path(f.getAbsolutePath).toString
  ...
  protected def createFileOnOCI(
      reservoirBase: String,
      filePath: String,
      file: FileOnOCI,
      clock: ManualClock,
      partitionValues: Map[String, String] = Map.empty): AddFile = {
    writeOnOCI(file, "gibberish")
    setLastModifiedOnOCI(file, clock.getTimeMillis())
  ...
  protected def gcTestOnOCI(deltaLog: DeltaLog, clock: ManualClock)(actions: Action*): Unit = {
    import testImplicits._
    val basePath = new Path(deltaLog.dataPath.toUri)
    val baseDir = new FileOnOCI(basePath)
    ...
    actions.foreach {
      case CreateFile(path, commit, partitionValues) =>
        ...
        val file = new FileOnOCI(baseDir, sanitizedPath)
        if (commit) {
          if (!DeltaTableUtils.isDeltaTable(spark, new Path(basePath.toString))) {
            // initialize the table
            deltaLog.startTransaction().commitManually()
          }
          val txn = deltaLog.startTransaction()
          val action = createFileOnOCI(basePath.toString, sanitizedPath, file, clock,
            partitionValues)
          txn.commit(Seq(action), Write(SaveMode.Append))
        } else {
          createFileOnOCI(basePath.toString, path, file, clock)
        }
      case CreateDirectory(path) =>
        Given(s"*** Creating directory at $path")
        val dir = new FileOnOCI(baseDir, path)
        assert(mkdirOnOCI(dir), s"Couldn't create directory at $path")
        assert(setLastModifiedOnOCI(dir, clock.getTimeMillis()))
      ...
      case CheckFiles(paths, exist) =>
        ...
        paths.foreach { p =>
          ...
          val f = new FileOnOCI(baseDir, sp)
          val res = if (exist) existsOnOCI(f) else !existsOnOCI(f)
      ...
      case ExpectFailure(action, failure, msg) =>
        ...
        val e = intercept[Exception](gcTestOnOCI(deltaLog, clock)(action))
  ...
  protected def vacuumSQLTestOnOCI(tablePath: String, identifier: String) {
    val deltaLog = DeltaLog.forTable(spark, tablePath)
    val committedFile = "committedFile.txt"
    val notCommittedFile = "notCommittedFile.txt"

    gcTestOnOCI(deltaLog, new ManualClock())(
      ...
      ExecuteVacuumInSQL(
        identifier,
        expectedDf = Seq(new FileOnOCI(new FileOnOCI(new Path(tablePath)),
          notCommittedFile).path.toString),
        dryRun = true),
      CheckFiles(Seq(committedFile, notCommittedFile)),
  ...
  test("vacuum for a partition path") {
    withEnvironmentOnOCI { (tempDir, _) =>
      import testImplicits._
  ...
  // Redundant for OCI
  ignore("vacuum a non-existent path and a non Delta table") {
...
class DeltaVacuumSuiteOnOCI
  extends DeltaVacuumSuiteBaseOnOCI with DeltaSQLCommandTest
