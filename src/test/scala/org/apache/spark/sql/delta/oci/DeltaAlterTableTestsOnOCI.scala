...
trait DeltaAlterTableTestBaseOnOCI
...
trait DeltaAlterTableTestsOnOCI extends DeltaAlterTableTestBaseOnOCI {
...
trait DeltaAlterTableByNameTestsOnOCI extends DeltaAlterTableTestsOnOCI {
  import testImplicits._
  ...
  // Redundant for OCI
  ignore("SET LOCATION - negative cases") {
    withTable("delta_table") {
      spark.range(1).write.format("delta").saveAsTable("delta_table")
      withTempDir { dir =>
        val path = dir.getCanonicalPath
 ...
 * resolution.
 */
trait DeltaAlterTableByPathTestsOnOCI extends DeltaAlterTableTestBaseOnOCI {
  override protected def createTable(schema: String, tblProperties: Map[String, String]): String = {
      val tmpDir = createTempDirOnOCI().getCanonicalPath
  ...
  override protected def dropTable(identifier: String): Unit = {
    deleteRecursivelyOnOCI(new FileOnOCI(new Path(identifier.stripPrefix("delta.`").
      stripSuffix("`"))))
  }
...
class DeltaAlterTableByNameSuiteOnOCI
  extends DeltaAlterTableByNameTestsOnOCI
...
class DeltaAlterTableByPathSuiteOnOCI extends DeltaAlterTableByPathTestsOnOCI
  with DeltaSQLCommandTest
