...
class DeltaInsertIntoSQLSuiteOnOCI extends DeltaInsertIntoTestsOnOCI(false, true)
...
class DeltaInsertIntoSQLByPathSuiteOnOCI extends DeltaInsertIntoTestsOnOCI(false, true)
  with DeltaSQLCommandTest {
  override protected def doInsert(tableName: String, insert: DataFrame, mode: SaveMode): Unit = {
    val tmpView = "tmp_view"
    withTempView(tmpView) {
    ...
    Seq(SaveMode.Append, SaveMode.Overwrite).foreach { mode =>
      withTempDirOnOCI { dir =>
        ...
        withTempView(tmpView) {
          ...
          assert(mkdirOnOCI(new FileOnOCI(dir, "_delta_log")),
...
class DeltaInsertIntoDataFrameSuite extends DeltaInsertIntoTestsOnOCI(false, false)
  with DeltaSQLCommandTest {
  override protected def doInsert(tableName: String, insert: DataFrame, mode: SaveMode): Unit = {
...
class DeltaInsertIntoDataFrameByPathSuite extends DeltaInsertIntoTestsOnOCI(false, false)
  with DeltaSQLCommandTest {
  override protected def doInsert(tableName: String, insert: DataFrame, mode: SaveMode): Unit = {
    val dfw = insert.write.format(v2Format)
    if (mode != null) {
...
abstract class DeltaInsertIntoTestsOnOCI (
