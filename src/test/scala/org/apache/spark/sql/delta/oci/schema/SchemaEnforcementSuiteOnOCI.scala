...
sealed trait SaveOperationOnOCI {
  def apply(dfw: DataFrameWriter[_]): Unit
}

case class SaveWithPathOnOCI(path: String = null) extends SaveOperationOnOCI {
  override def apply(dfw: DataFrameWriter[_]): Unit = {
    if (path == null) dfw.save() else dfw.save(path)
  }
}

case class SaveAsTableOnOCI(tableName: String) extends SaveOperationOnOCI {
  override def apply(dfw: DataFrameWriter[_]): Unit = dfw.saveAsTable(tableName)
}

sealed trait SchemaEnforcementSuiteBaseOnOCI extends QueryTestOnOCI {
...
sealed trait BatchWriterTestOnOCI extends SchemaEnforcementSuiteBaseOnOCI {

  def saveOperation: SaveOperationOnOCI

  implicit class RichDataFrameWriter(dfw: DataFrameWriter[_]) {
    def append(path: FileOnOCI): Unit = {
      saveOperation(dfw.format("delta").mode("append").option("path", path.getAbsolutePath))
    }

    def overwrite(path: FileOnOCI): Unit = {
  ...
  def equivalenceTest(testName: String)(f: => Unit): Unit = {
    test(s"batch: $testName") {
      saveOperation match {
        case _: SaveWithPathOnOCI => f
        case SaveAsTableOnOCI(tbl) => withTable(tbl) { f }
      }
    }
  }
}

trait AppendSaveModeTestsOnOCI extends BatchWriterTestOnOCI {
  ...
  equivalenceTest("ensure schema mismatch error message contains table ID") {
    disableAutoMigration {
      withTempDirOnOCI { dir =>
        ...
        assert(e.getMessage.contains(s"Table ID: ${DeltaLog.forTable(spark, dir.path).tableId}"))
      }
    }
  }
}

trait AppendOutputModeTestsOnOCI extends SchemaEnforcementSuiteBaseOnOCI with SQLTestUtils {
  import testImplicits._
...
trait OverwriteSaveModeTestsOnOCI extends BatchWriterTestOnOCI {
  import testImplicits._
...
trait CompleteOutputModeTestsOnOCI extends SchemaEnforcementSuiteBaseOnOCI with SQLTestUtils {
  import testImplicits._
...
class SchemaEnforcementWithPathSuiteOnOCI extends AppendSaveModeTestsOnOCI
  with OverwriteSaveModeTestsOnOCI {
  override val saveOperation = SaveWithPathOnOCI()
}

class SchemaEnforcementWithTableSuiteOnOCI
  extends AppendSaveModeTestsOnOCI
  with OverwriteSaveModeTestsOnOCI
  with DeltaSQLCommandTest {

  override val saveOperation = SaveAsTableOnOCI("delta_schema_test")
}

class SchemaEnforcementStreamingSuiteOnOCI
  extends AppendOutputModeTestsOnOCI
  with CompleteOutputModeTestsOnOCI {
}
