...
import org.apache.spark.sql.delta.{DeltaLog, DeltaOperationMetrics, DeltaOperations, DeltaOptions}
...
trait DescribeDeltaHistorySuiteBaseOnOCI
  ...
  testWithFlag("operations - ctas (sql)") {
    val tempDir_d = createTempDirOnOCI()
    val tempDir2_d = createTempDirOnOCI()
    val tempDir = tempDir_d.toString
    val tempDir2 = tempDir2_d.toString
    try {
      withTable("delta_test") {
        sql(
          s"""create table delta_test
      ...
      deleteRecursivelyOnOCI(tempDir_d)
      deleteRecursivelyOnOCI(tempDir2_d)
    }
  }
  ...
  test("operations - upgrade protocol") {
    withTempDirOnOCI { path =>
      val log = DeltaLog.forTable(spark, path.toString)
  ...
  test("operations - streaming append with transaction ids") {
    val tempDir_d = createTempDirOnOCI()
    val tempDir = tempDir_d.toString
    val checkpoint_d = createTempDirOnOCI()
    val checkpoint = checkpoint_d.toString
    try {
      ...
      deleteRecursivelyOnOCI(tempDir_d)
      deleteRecursivelyOnOCI(checkpoint_d)
    }
  }
  ...
  testWithFlag("operations - insert overwrite with predicate") {
    ...
    val tempDir = tempDir_d.toString
    try {
      Seq((1, "a"), (2, "3")).toDF("id", "data").write.format("delta").partitionBy("id").
        save(tempDir)
  ...
  testWithFlag("order history by version") {
    ...
    try {
      ...
      val ans =
        io.delta.tables.DeltaTable.forPath(spark, tempDir).history().as[CommitInfo].collect()
  ...
  test("operation metrics - convert to delta") {
    withSQLConf(DeltaSQLConf.DELTA_HISTORY_METRICS_ENABLED.key -> "true") {
      ...
      withTempDirOnOCI { tempDir =>
        ...
        val dir = tempDir.toString // tempDir.getAbsolutePath()
        spark.range(10)
...
class DescribeDeltaHistorySuiteOnOCI
  extends DescribeDeltaHistorySuiteBaseOnOCI with DeltaSQLCommandTest
