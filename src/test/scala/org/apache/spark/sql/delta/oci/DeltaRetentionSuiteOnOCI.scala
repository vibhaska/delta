...
class DeltaRetentionSuiteOnOCI extends QueryTestOnOCI with DeltaRetentionSuiteBaseOnOCI
  with SQLTestUtils {

  protected override def sparkConf: SparkConf = super.sparkConf

  override protected def getLogFiles(dir: FileOnOCI): Seq[FileOnOCI] = {
    val files: Seq[FileOnOCI] = getDeltaFiles(dir) ++ getCheckpointFiles(dir)
  ...
  test("delete expired logs") {
    withTempDirOnOCI { tempDir =>
      ...
      val logPath = new FileOnOCI(log.logPath.toUri)
      (1 to 5).foreach { i =>
  ...
  test("log files being already deleted shouldn't fail log deletion job") {
    withTempDirOnOCI { tempDir =>
      ...
      (1 to 25).foreach { i =>
        ...
        val deltaFile = new FileOnOCI(FileNames.deltaFile(log.logPath, version).toUri)
        setLastModifiedOnOCI(deltaFile, clock.getTimeMillis() + i * 10000)
        val crcFile = new FileOnOCI(FileNames.checksumFile(log.logPath, version).toUri)
        setLastModifiedOnOCI(crcFile, clock.getTimeMillis() + i * 10000)
        val chk = new FileOnOCI(FileNames.checkpointFileSingular(log.logPath, version).toUri)
        if (existsOnOCI(chk)) {
          setLastModifiedOnOCI(chk, clock.getTimeMillis() + i * 10000)
        }
      }

      // delete some files in the middle
      getDeltaFiles(logPath).sortBy(_.getName).slice(5, 15).foreach(deleteOnOCI(_))
      clock.advance(intervalStringToMillis(DeltaConfigs.LOG_RETENTION.defaultValue) +
