...
trait DeltaTableCreationTestsOnOCI
  extends QueryTestOnOCI
      ...
      test(s"saveAsTable to a new table (external) - $isPartitioned, saveMode: $saveMode") {
        withTempDirOnOCI { dir =>
          ...
          withTable(tbl) {
            ...
            checkDatasetUnorderly(spark.table(tbl).as[(Long, String)], 1L -> "a")
            assert(getTablePath(tbl) === new Path(dir.toString).toString.stripSuffix("/"),
              "Table path is wrong")
            assert(getPartitioningColumns(tbl) === cols, "Partitioning columns don't match")
          }
        }
  ...
  testQuietly("saveAsTable (overwrite) to a non-partitioned table created with different paths") {
    withTempDirOnOCI { dir1 =>
      withTempDirOnOCI { dir2 =>
        withTable("delta_test") {
          Seq(1L -> "a").toDF("v1", "v2")
            .write
            .mode(SaveMode.Append)
  ...
  Seq(true, false).foreach { shouldDelete =>
    ...
    test(s"CTAS for external data source table with $tcName location") {
      ...
      withTable("t", "t1") {
        withTempDirOnOCI { dir =>
          if (shouldDelete) deleteOnOCI(dir)
          spark.sql(
            s"""
               |CREATE TABLE t
               |USING delta
    ...
    test(s"data source table:partition column name containing $specialChars") {
      ...
      withTable("t") {
        withTempDirOnOCI { dir =>
          ...
          assert(listFilesOnOCI(dir).forall(_.toString.contains("_delta_log")))
          spark.sql(s"INSERT INTO TABLE t SELECT 1, 2")
          val partEscaped = s"${ExternalCatalogUtils.escapePathName(specialChars)}=2"
          val partFile = new FileOnOCI(dir, partEscaped)
          assert(listFilesOnOCI(partFile).nonEmpty)
  ...
  Seq("a b", "a:b", "a%b").foreach { specialChars =>
    test(s"location uri contains $specialChars for datasource table") {
      ...
      withTable("t", "t1") {
        withTempDirOnOCI { dir =>
          val loc = new FileOnOCI(dir, specialChars)
          mkdirOnOCI(loc)
          ...
          assert(listFilesOnOCI(loc).forall(_.toString.contains("_delta_log")))
        ...
        withTempDirOnOCI { dir =>
          ...
          val partFile = new FileOnOCI(loc, "b=2")
          assert(!listFilesOnOCI(partFile).forall(_.toString.contains("_delta_log")))
          checkAnswer(spark.table("t1"), Row("1", "2") :: Nil)

          spark.sql("INSERT INTO TABLE t1 SELECT 1, '2017-03-03 12:13%3A14'")
          val partFile1 = new FileOnOCI(loc, "b=2017-03-03 12:13%3A14")
          assert(!existsOnOCI(partFile1))

          if (!Utils.isWindows) {
            // Actual path becomes "b=2017-03-03%2012%3A13%253A14" on Windows.
            val partFile2 = new FileOnOCI(loc, "b=2017-03-03 12%3A13%253A14")
            assert(!listFilesOnOCI(partFile2).forall(_.toString.contains("_delta_log")))
            checkAnswer(
      ...
      withTable("t", "t1") {
        ...
        spark.sql(
          s"""
           ...
           """.stripMargin)
        val table = spark.sessionState.catalog.getTableMetadata(TableIdentifier("t"))
        assert(table.location.toString.startsWith("oci:/"))
      }
    }
  ...
  test("CREATE TABLE on existing data should not commit metadata") {
    withTempDirOnOCI { tempDir =>
      ...
      df.write.format("delta").save(path.toString)
      val deltaLog = getDeltaLog(new Path(path.toString))
...
class DeltaTableCreationSuiteOnOCI
  extends DeltaTableCreationTestsOnOCI
