...
import org.apache.spark.sql.delta.{CheckpointInstance, DeltaErrors, DeltaLog, DeltaOperations}
import org.apache.spark.sql.oci.{FileOnOCI, QueryTestOnOCI}
import org.apache.spark.sql.test.{SQLTestUtils, SharedSparkSession}
import org.apache.spark.util.Utils

// scalastyle:off: removeFile
class DeltaLogSuiteOnOCI extends QueryTestOnOCI
  ...
  // Ignored for OCI
  ignore("ActionLog cache should use the normalized path as key") {
    withTempDirOnOCI { tempDir =>
  ...
  testQuietly("paths should be canonicalized") {
    Seq("file:", "file://").foreach { scheme =>
      withTempDirOnOCI { dir =>
        val log = DeltaLog.forTable(spark, dir.path)
        assert(mkdirOnOCI(new FileOnOCI(log.logPath.toUri)))
  ...
  test("do not relativize paths in RemoveFiles") {
    withTempDirOnOCI { dir =>
      val log = DeltaLog.forTable(spark, dir.path)
      assert(mkdirOnOCI(new FileOnOCI(log.logPath.toUri)))
      val path = new FileOnOCI(dir, "a/b/c").getCanonicalPath
      ...
      assert(committedRemove.head.path === s"$path")
    }
  }
  ...
  test("error - versions not contiguous") {
    withTempDirOnOCI { dir =>
      ...
      deleteOnOCI(new FileOnOCI(new Path(log.logPath, "00000000000000000001.json").toUri))

      DeltaLog.clearCache()
      val ex = intercept[IllegalStateException] {
        DeltaLog.forTable(spark, dir.path)
      }
  ...
  Seq("protocol", "metadata").foreach { action =>
    testQuietly(s"state reconstruction from checkpoint with missing $action should fail") {
      withTempDirOnOCI { tempDir =>
        ...
        val log = DeltaLog.forTable(spark, tempDir.path)
        val checkpointInterval = log.checkpointInterval
...

        {
          ...
          withTempDirOnOCI { tmpCheckpoint =>
            ...
            val writtenCheckpoint =
              listFilesOnOCI(tmpCheckpoint).toSeq.filter(_.getName.startsWith("part")).head
            val checkpointFile = new FileOnOCI(checkpointPath.toUri)
            listFilesOnOCI(new FileOnOCI(log.logPath.toUri)).toSeq.foreach { file =>
              if (file.getName.startsWith(".0")) {
                ...
                require(deleteOnOCI(file), "Failed to delete checksum file")
              }
            }
            require(deleteOnOCI(checkpointFile), "Failed to delete old checkpoint")
            require(renameToOnOCI(writtenCheckpoint, checkpointFile),
        ...
        withSQLConf(DeltaSQLConf.DELTA_STATE_RECONSTRUCTION_VALIDATION_ENABLED.key -> "true") {
          val e = intercept[IllegalStateException] {
            DeltaLog.forTable(spark, tempDir.path).update()
          }
        ...
        withSQLConf(DeltaSQLConf.DELTA_STATE_RECONSTRUCTION_VALIDATION_ENABLED.key -> "false") {
          assert(DeltaLog.forTable(spark, tempDir.path).update().version === 10)
        }
      }
    }
  }
  ...
  test("deleting and recreating a directory should cause the snapshot to be recomputed") {
    withTempDirOnOCI { dir =>
      ...
      // second
      setLastModifiedOnOCI(new FileOnOCI(FileNames.deltaFile(deltaLog.logPath, 0).toUri),
